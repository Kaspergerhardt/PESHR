...
#     axs[0].set_title("Spectra van RC's")
...
#     axs[1].set_title("Spectra van RC-paren")
...
def plot_ssa_overview(s, RCs, rc_pairs, pair_scores, fs, chosen_pair, pair_to_highlight=None, n_values=10):
    ...
    for i in range(min(10, RCs.shape[0])):
        x = RCs[i] - np.mean(RCs[i])
        freqs_hz = np.fft.rfftfreq(len(x), d=1 / fs)
        freqs_bpm = freqs_hz * 60

        psd = np.abs(np.fft.rfft(x)) ** 2
        mask = (freqs_bpm >= 24) & (freqs_bpm <= 240)

        axs[0].plot(freqs_bpm[mask], psd[mask], label=f"RC {i}")
...
def plot_cgo_analysis(
    ...
    fig.suptitle("CGO-analyse en hartslagschatting", fontsize=14)
    ...
    # 2. Bandgefilterd PES
    # ---------------------------
    axes[1].plot(time_window, pes_bp, label="Bandgefilterd PES", alpha=0.7)
    axes[1].set_title("Bandgefilterd PES-signaal")
    ...
    # ------------------------
    axes[2].plot(time_window, cgo, label="Geëxtraheerde CGO", linewidth=2)
    ...
    axes[2].set_title("Geëxtraheerde CGO met piekdetectie")
    ...
    # -------------------------------------------
    axes[3].plot(time_window, pes_window, label="Origineel PES", alpha=0.7)
    ...
    axes[3].set_title("Origineel PES met gedetecteerde pieken (CGO)")
    ...
    axes[4].set_title("Hartslag over 60 s")
...
def bland_altman_plot(
    ...
    col_ref : str
        Reference column (gold standard)
    col_test : str
        Test method column
    sep : str
        CSV separator
    title : str
        Plot title
    filter_condition : pandas boolean mask
        Optional filtering condition (e.g. df["chosen_cgo"] == True)
    show_plot : bool
    ...
    plt.scatter(mean_vals, diff_vals, alpha=0.8)
    ...
    plt.axhline(3, linestyle="-.", label="+3 bpm")
    plt.axhline(-3, linestyle="-.", label="-3 bpm")
...
def plot_clinical_impact_distribution(group_a, group_b, save_path=None):
    plt.figure(figsize=(8, 5))

    plt.hist(
        group_a["clinical_impact"].dropna(),
        bins=15,
        alpha=0.6,
        density=True,
        label="Mismatch + EIT match",
    )

    plt.hist(
        group_b["clinical_impact"].dropna(),
        bins=15,
        alpha=0.6,
        density=True,
        label="Remaining",
    ...
    plt.xlabel("Clinical impact")
    plt.ylabel("Density")
    plt.title("Distribution of clinical impact")
    plt.legend()
    plt.grid(True)
...
def plot_clinical_impact_boxplot(group_a, group_b, save_path=None):
    ...
    plt.boxplot(
        ...
        tick_labels=["Mismatch + EIT match", "Remaining"],
    )

    plt.ylabel("Clinical impact")
    plt.title("Clinical impact comparison")
    plt.grid(True)
...
def bland_altman_plot(
    ...
    if filter_condition is not None:
        df = df[filter_condition(df)].copy()

    mean_values = (df[col_ref] + df[col_test]) / 2
    diff_values = df[col_test] - df[col_ref]
    ...
    if highlight_condition is None:
        plt.scatter(mean_values, diff_values, alpha=0.7, label="All cases")
    else:
        highlight_mask = highlight_condition(df)

        plt.scatter(
            mean_values[~highlight_mask],
            diff_values[~highlight_mask],
            alpha=0.6,
            label="Non-mismatch",
        )
    ...
    plt.xlabel(f"Mean of {col_ref} and {col_test} [bpm]")
    plt.ylabel(f"{col_test} - {col_ref} [bpm]")
    plt.title(title)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
